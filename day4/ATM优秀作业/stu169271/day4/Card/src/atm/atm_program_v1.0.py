...
def login(username, account_name, password):
    ...
    while flag:
        if count < 3:                           # 只要重试不超过三次就不断循环
            ...
            user_name = card_user_info_dict[username].get('account_name')
            pass_word = card_user_info_dict[username].get('password')
            agree_flag = card_user_info_dict[username].get('agree_flag')
            account_stat = card_user_info_dict[username].get('account_stat')
...
def card_register_user():
    ...
    card_user_info["card_limit"] = 0                # 用户信用额度，默认额度为0，审批通过然后进行额度授权
    card_user_info["card_limit_surplus"] = 0        # 用户信用额度余额
    card_user_info["card_cash_limit"] = 0           # 用户取现额度
    card_user_info["card_cash_limit_surplus"] = 0   # 用户取现额度余额
...
def audit_user_register():
    ...
    for i in card_user_info_dict.keys():
        user_info_msg = """
姓名：%s\t\t手机号:%s\t\t邮箱：%s\t\t地区：%s\t\t审批状态：%s\t\t用户额度：%s\t\t取现额度：%s
        """ % (card_user_info_dict[i].get('user_name'),
               ...
               card_user_info_dict[i].get('card_cash_limit'),)
 ...
 姓名：%s\t\t手机号:%s\t\t邮箱：%s\t\t地区：%s\t\t审批状态：%s\t\t用户额度：%s\t\t取现额度：%s
        ...
        if card_agree_limit.strip().isdigit():
            card_agree_limit = int(card_agree_limit)
        ...
        card_user_info_dict[card_user_agree]['card_cash_limit'] = card_agree_limit/2
        card_user_info_dict[card_user_agree]['card_cash_limit_surplus'] = card_agree_limit/2
...
姓名：%s\t\t手机号:%s\t\t邮箱：%s\t\t地区：%s\t\t审批状态：%s\t\t用户额度：%s
        """ % (card_user_info_dict[i].get('user_name'),
               ...
               card_user_info_dict[i].get('card_limit'),)
        print(user_info_msg.strip("\n"))
    card_limit_user = input("请输入要调整额度的用户名(非账号)：")  # 要完善退出功能

    card_limit_sum = input("请输入要调整的额度：")
    if card_limit_sum.strip().isdigit():
        card_limit_sum = int(card_limit_sum)
    card_user_info_dict[card_limit_user]['card_limit'] = \
        int(card_user_info_dict[card_limit_user]['card_limit']) + card_limit_sum
    card_user_info_dict[card_limit_user]['card_cash_limit'] = int(card_user_info_dict[card_limit_user]['card_limit'])/2
    pickle.dump(card_user_info_dict, open(user_info_file, 'wb'))  # 将审批完成的信息写会文件
...
def frozen_account_manager():
    ...
    :return:
    """
    account_name = input("请输入要冻结的用户名(非账号)：")
    card_user_info_dict = pickle.load(open(user_info_file, 'rb'))  # 获取原有的用户信息
    card_user_info_dict[account_name]['account_stat'] = 1
    pickle.dump(card_user_info_dict, open(user_info_file, 'wb'))
    print(color_green + "该用户已经被冻结....." + color_end)
...
=========================================
                            """ % (card_user_info_dict[i]['card_limit'],
                                   card_user_info_dict[i]['card_cash_limit'],
                                   card_user_info_dict[i]["card_limit_surplus"],
                                   card_user_info_dict[i]["card_cash_limit_surplus"],)
            print(user_info_msg.strip("\n"))
            current_user = card_user_info_dict[i]['user_name']
    # print(current_user)
    card_cash_sum = input("请输入要取现的金额：")
    if card_cash_sum.strip().isdigit():
        card_cash_sum = int(card_cash_sum)
    if card_user_info_dict[current_user]["card_cash_limit_surplus"] > card_cash_sum:
        a = card_cash_sum * 0.05
        print("取现成功，取现手续费%.2f元" % a)
        card_user_info_dict[current_user]["card_cash_limit_surplus"] = \
            card_user_info_dict[current_user]["card_cash_limit_surplus"] - card_cash_sum - a
        pickle.dump(card_user_info_dict, open(user_info_file, 'wb'))  # 将审批完成的信息写会文件
    else:
...
=========================================
    ...
    if transfer_to_user in card_user_info_dict.keys():
        ...
        card_user_info_dict[current_user]["card_cash_limit_surplus"] = \
            card_user_info_dict[current_user]["card_cash_limit_surplus"] - transfer_to_sum
        card_user_info_dict[transfer_to_user]["card_cash_limit_surplus"] = \
            card_user_info_dict[transfer_to_user]["card_cash_limit_surplus"] + transfer_to_sum
        pickle.dump(card_user_info_dict, open(user_info_file, 'wb'))  # 将审批完成的信息写会文件
    card_user_info_dict = pickle.load(open(user_info_file, 'rb'))  # 获取用户信息
    current_user = ""
    for i in card_user_info_dict.keys():
...
=========================================
    """
        ...
        if choice_num == 1:
            ...
            username = input("姓名(非账号)：")
            account_name = input("账号：")
            password = input("密码：")
            login(username, account_name, password)
        elif choice_num == 3:
            credit_card_limit()
        elif choice_num == 4:
            card_get_cash()
